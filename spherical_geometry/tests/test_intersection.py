...
# LOCAL
from spherical_geometry.polygon import SphericalPolygon, _SingleSphericalPolygon
    ...
    def __call__(self, func):
        ...
        def run(*args, **kwargs):
            ...
            for i, permutation in enumerate(
                ...
                intersection = SphericalPolygon.multi_intersection(permutation)
...
def test1():
    ...
    poly1 = SphericalPolygon.from_wcs(header, 1, crval=[0, 87])
    poly2 = SphericalPolygon.from_wcs(header, 1, crval=[20, 89])
...
@intersection_test(0, 90)
def test2():
    poly1 = SphericalPolygon.from_cone(0, 60, 8, steps=16)
    poly2 = SphericalPolygon.from_cone(0, 68, 8, steps=16)
    poly3 = SphericalPolygon.from_cone(12, 66, 8, steps=16)
...
def test3():
    ...
    poly1 = SphericalPolygon.from_wcs(header, 1, crval=[0, 87])
    poly3 = SphericalPolygon.from_wcs(header, 1, crval=[175, 89])
...
def test_intersection_empty():
    ...
    p2 = p.intersection(SphericalPolygon([]))
...
def test_self_intersection():
    ...
    s1 = SphericalPolygon.from_radec(np.array(ra1), np.array(dec1))
    s2 = SphericalPolygon.from_radec(np.array(ra2), np.array(dec2))
    s12 = s2.union(s1)
    ...
    s12int = SphericalPolygon.multi_intersection([s12, s12, s12])
    assert abs(s12.area() - s12int.area()) < 1.0e-6
...
def test_ordering():
    ...
    def roll_polygon(P, i):
        ...
        for p in P.polygons:
            ...
            p = _SingleSphericalPolygon(points, p.inside)
            polygons.append(p)
        return SphericalPolygon(polygons)
...
@pytest.mark.parametrize(
    ...
    [
        [
            SphericalPolygon.from_lonlat(lon=lon, lat=lat, degrees=False)
            for (lon, lat) in [
