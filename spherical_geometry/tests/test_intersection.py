    ...
    def __call__(self, func):
        ...
        def run(*args, **kwargs):
            ...
            for i, permutation in enumerate(
                ...
                intersection = polygon.SphericalPolygon.multi_intersection(permutation)
                intersections.append(intersection)
                intersection_area = intersection.area()
                if GRAPH_MODE:
                    fig = plt.figure()
                    m = Basemap(projection=self._proj, lon_0=self._lon_0, lat_0=self._lat_0)
            ...
            lengths = np.array([sum(len(x._points) for x in y) for y in intersections])
...
def test_intersection_empty():
    p = polygon.SphericalPolygon.from_cone(
        random.randrange(-180, 180), random.randrange(20, 90), random.randrange(5, 16), steps=16
    )
...
def test_difficult_intersections():
    ...
    for i in range(0, len(lines), 4):
        Apoints, Ainside, Bpoints, Binside = [to_array(line) for line in lines[i : i + 4]]
